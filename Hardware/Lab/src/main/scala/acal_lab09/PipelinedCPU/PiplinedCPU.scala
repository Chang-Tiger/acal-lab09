...
class PiplinedCPU(memAddrWidth: Int, memDataWidth: Int) extends Module {
    ...
    /* Wire Connect */
    // === IF stage reg (PC reg) ======================================================
    stage_IF.io.Stall := contorller.io.Stall_WB_ID_DH && (~contorller.io.Flush_BH) // if no Branch Miss, stall when Data Hazrd
    ...
    // === EXE stage reg ==============================================================
    stage_EXE.io.Flush := (contorller.io.Flush_WB_ID_DH || contorller.io.Flush_BH) // Flush when Data Hazard or Branch Misss
    stage_EXE.io.Stall := contorller.io.Hcf   // not used
    ...
    // === MEM stage reg ==============================================================
    stage_MEM.io.Stall := contorller.io.Hcf        // not used
    ...
    // === WB stage reg ==============================================================
    stage_WB.io.Stall := contorller.io.Hcf        // not used
    ...
    // connect Branch Miss for output
    io.Flush := contorller.io.Flush_BH
    // connect Data Hazard for output
    io.Stall_DH := contorller.io.Stall_WB_ID_DH
    // connect Memory Access for output
