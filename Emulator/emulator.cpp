...
instr_type parse_instr(char *tok)
{
	...
	if (streq(tok, "vse8_v"))
		return VSE8_V;
	if (streq(tok, "vadd_vv"))
		return VADD_VV;
	if (streq(tok, "vmul_vx"))
		return VMUL_VX;
	//*****************
...
void execute(uint8_t *mem, instr *imem, label_loc *labels, int label_count, bool start_immediate)
{
	...
	while (!dexit)
	{
		...
		case MINU:
		{
			rf[i.a1.reg] = ((uint32_t)rf[i.a2.reg] < (uint32_t)rf[i.a3.reg]) ? (uint32_t)rf[i.a2.reg] : (uint32_t)rf[i.a3.reg];
			break;
		}
		case MAXU:
		{
		...
		case SEXTB:
		{
			int32_t idx = (int32_t)(rf[i.a2.reg] << 24);
			rf[i.a1.reg] = (uint32_t)(idx >> 24);
			break;
		}
		case SEXTH:
		{
			int32_t idx = (int32_t)(rf[i.a2.reg] << 16);
			rf[i.a1.reg] = (uint32_t)(idx >> 16);
			break;
		}
		case BSET:
		{
			rf[i.a1.reg] = rf[i.a3.reg] & (32 - 1);
			rf[i.a1.reg] = rf[i.a2.reg] | (1 << rf[i.a1.reg]);
			break;
		}
		case BCLR:
		{
